...
int UserManager::add_user(int opt_idx, const Username &cur_usr, const Username &new_usr, const Password &pwd, const Name &name, const MailAddr &maddr, int priv ) 
{
...
int UserManager::login(int opt_idx, const Username &usr, const Password &pwd)
{
    // user does not exist
    if(!user.count(usr)) {
...
int UserManager::logout(int opt_idx, const Username &usr) 
{
...
int UserManager::query_profile(int opt_idx, const Username &cur_usr, const Username &qry_usr) 
{
    ...
    if(cur_usr == qry_usr) {
        ...
        std::cout << "[" << opt_idx << "] ";
        std::cout << qry_usr << " " << info.name << " " << info.maddr << " " << info.pri << std::endl;
        return 0;
    }
    ...
    std::cout << "[" << opt_idx << "] ";
    std::cout << qry_usr << " " << qry_info.name << " " << qry_info.maddr << " " << qry_info.pri << std::endl;    
    return 0;
}

int UserManager::modify_profile(int opt_idx, const Username &cur_usr, const Username &mod_usr, const Password &mod_pwd, const Name &mod_name,const MailAddr &mod_maddr, int mod_priv) 
{
    if(!online[cur_usr]) {
        throw user_error("current user does not exist or hasn't logged in");
    }
    ...
    std::cout << "[" << opt_idx << "] ";
    std::cout << mod_usr << " " << info.name << " " << info.maddr << " " << info.pri << std::endl;    
    return 0;
}
